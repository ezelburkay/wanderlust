...
const quickFactLabels = [
  { key: "bestMonths", label: "Best in" },
  { key: "worksBestFor", label: "Works best for" },
  { key: "bestFor", label: "Especially strong for" },
  { key: "idealDays", label: "Ideal rhythm" }
] as const satisfies ReadonlyArray<{
  key: keyof CityViewModel["quickFacts"];
  label: string;
}>;

type VisibleQuickFact = {
  key: (typeof quickFactLabels)[number]["key"];
  label: string;
  value: string;
};
...
function getVisibleQuickFacts(city: CityViewModel): VisibleQuickFact[] {
  return quickFactLabels.flatMap(({ key, label }) => {
    const value = city.quickFacts?.[key];
...
export default function CityPage({ params }: CityPageProps) {
  const city = getRequiredCity(params.slug);
  const quickFacts = getVisibleQuickFacts(city);
